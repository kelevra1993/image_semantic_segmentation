...
def create_ellipse_data(
    ...
    cv2.ellipse(img=ground_truth_numpy_array, center=ellipse_center, axes=axes_length, angle=0, startAngle=0, endAngle=360, color=1, thickness=-1)
...
def create_input_data(
    ...
    logit_dictionary: Dict[str, Dict[str, float]]
    ...
    Args:
        ...
        logit_dictionary (Dict[str, Dict[str, float]]): A dictionary specifying the prediction logits 
            for each class. Must include 'background_logit' as a float, and nested dicts for 'circle', 
            'square', 'triangle', and 'ellipse' with 'left_logit' and 'right_logit'.
    ...
    prediction_tensor = torch.full((batch_size, number_of_classes, image_height, image_width), inactive_logit, dtype=torch.float32)
    
    # 0. Background
    background_logit = logit_dictionary.get("background_logit", 0.0)
    if isinstance(background_logit, dict): # robust check in case it's misformatted
        background_logit = background_logit.get("left_logit", 0.0) 
    prediction_tensor[0, 0, :, :] = float(background_logit)
    
    # 1. Circle (Top-Left)
    circle_left = logit_dictionary["circle"]["left_logit"]
    circle_right = logit_dictionary["circle"]["right_logit"]
    circle_ground_truth, circle_prediction = create_circle_data(quadrant_width, quadrant_height, circle_left, circle_right, inactive_logit)
    ...
    square_ground_truth, square_prediction = create_square_data(quadrant_width, quadrant_height, square_left, square_right, inactive_logit)
    ...
    triangle_ground_truth, triangle_prediction = create_triangle_data(quadrant_width, quadrant_height, triangle_left, triangle_right, inactive_logit)
    ...
    ellipse_ground_truth, ellipse_prediction = create_ellipse_data(quadrant_width, quadrant_height, ellipse_left, ellipse_right, inactive_logit)
    ground_truth_tensor[0, 4, quadrant_height:image_height, quadrant_width:image_width] = torch.from_numpy(ellipse_ground_truth)
    prediction_tensor[0, 4, quadrant_height:image_height, quadrant_width:image_width] = torch.from_numpy(ellipse_prediction)
